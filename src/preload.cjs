...
contextBridge.exposeInMainWorld('xpmd', {
  ...
  saveMarkdown: (content) => ipcRenderer.invoke('editor:save', content),
  saveMarkdownAs: (content) => ipcRenderer.invoke('editor:saveAs', content),
  setEditing: (value) => ipcRenderer.invoke('editor:setEditing', value),
  onEditCommand: (callback) => ipcRenderer.on('edit-command', (_event, command) => callback(command)),
